...
export default function SignupPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Store the JWT token in localStorage if returned
      if (data.data?.access_token) {
        ...
        // If signup succeeded but no token (might require login), redirect to login
        router.push('/login');
      }
    } catch (err: any) {
          ...
          <Card animated={true} title="Join TalkTodo">
            ...
            <div className="mt-4 text-center text-sm text-[rgb(var(--muted-foreground))]">
              <p>
                ...
                <Link href="/login" className="font-medium text-[rgb(var(--primary))] hover:text-[rgb(var(--primary)/0.8)]">
                  Sign in
                </Link>
              </p>
            </div>
